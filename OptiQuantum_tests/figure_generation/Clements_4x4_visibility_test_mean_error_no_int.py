...
def main():
    ...
    N = 4
    #Create Mesh
    network = ClementsLayer(N, phases=[(np.pi, np.pi) for _ in range(int(N*(N-1)/2))], phase_uncert=np.pi/32)
    mesh = network.mesh
    ...
    for layer in mesh.layers:
        for mzi in layer:
            ...
            plt.savefig(f'figures_set1/Update October 26/visibility_vs_mean_phases_Clements_1000samples_MZI_{n}.png')

            #Save Data
            np.savetxt(f'figures_set1/Update October 26/visibiity_vs_mean_phases_Clements_1000samples_MZI_{n}.txt',visibs)
