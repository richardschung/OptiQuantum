...
def main():
    ...
    layers[1].mzis[0].theta = np.pi/2
    layers[1].mzis[0].phi = 0
    ...
    for i in range(n_phase):
        for layer in mesh.layers:
            for mzi in layer:
                mzi.theta_err = phase_errs[1]
                mzi.phi_err = phase_errs[i]

        print(np.round(mesh.get_transfer_matrix(),2))
            
        intfunc_min = lambda omega1, omega2:HOM_integrand_mesh(mesh,omega1,omega2,0, 1, 1, 2, 1, 2)
        coinc_min = scipy.integrate.dblquad(intfunc_min,-np.inf,np.inf,-np.inf,np.inf)

        intfunc_max = lambda omega1, omega2:HOM_integrand_mesh(mesh,omega1,omega2,5, 1, 1, 2, 1, 2)
    ...
    plt.xlabel('Absolute Phi Error (Radians)')
