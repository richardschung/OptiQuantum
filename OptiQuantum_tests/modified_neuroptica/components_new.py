...
class MZI(OpticalComponent):
    ...
    def get_transfer_matrix(self, add_uncertainties=True) -> np.ndarray:
        ...
        mzi_r = 0.5 * np.array([
            ...
            1j * (1 + np.exp(1j * theta))],
            [1j * np.exp(1j * phi) * (np.exp(1j * theta) + 1), 1 - np.exp(1j * theta)]
            ], dtype=NP_COMPLEX)
...
#Modified to apply to all elements
def apply_loss(mzi, loss):
    return np.array([[loss, 0],[0, loss]]) @ mzi

#Modified to accept truncated norms
def get_loss(loss_dB, loss_diff=0, rv='trunc_gauss'):
    if rv == 'exp':
        loss_dB_cur = loss_dB + np.random.exponential(loss_diff)
        return loss_dB_cur
    elif rv == 'trunc_gauss':
        loss_dB_cur = scipy.stats.truncnorm.rvs(0, np.inf, loss_dB, loss_diff)
        return loss_dB_cur
    elif rv == 'gauss':
...
#New MZI class with photon delay
class MZI_delay(MZI):
    '''Simulation of a programmable phase-shifting Mach-Zehnder interferometer with temporal delay of pulse propagation'''

    def __init__(self, m: int, n: int, theta: float = None, phi: float = None, phase_uncert_theta=0.0, phase_uncert_phi=0.0, loss_dB=0, loss_diff=0, omega=0.0, delta_t=0.0):
        '''
        :param m: first waveguide index
        :param n: second waveguide index
        :param omega: optical frequency
        :param delta_t: difference in propagation time for identical photons in the upper and lower MZI branches at a given group velocity
        :param theta: phase shift value for inner phase shifter; assigned randomly between [0, 2pi) if unspecified
        :param phi: phase shift value for outer phase shifter; assigned randomly between [0, 2pi) if unspecified
        :param phase_uncertainty: optional uncertainty to add to the phase shifters; effective phase is computed as
        self.(theta, phi) + np.random.normal(0, self.phase_uncert) if add_uncertainties is set to True during simulation
        '''
        super().__init__(m, n, theta, phi, phase_uncert_theta, phase_uncert_phi, loss_dB, loss_diff)
        if omega is None: omega = 0.0
        if delta_t is None: delta_t = 0.0
        self.omega = omega # Photon frequency
        self.delta_t = delta_t # Temporal delay between the MZI branches
        self.loss_dB_cur = loss_dB # dB loss, held constant over single integration
        self.theta_err = 0 #Error in theta due to phase uncertainty, held constant over single integration
        self.phi_err = 0 #Error in due to phase uncertainty, held constant over single integration

    def __repr__(self):
        return '<MZI index: {}{}, theta={:.3f}, phi={:.3f}>'.format(self.m, self.n, self.theta, self.phi)

    def get_transfer_matrix(self, add_uncertainties=True, add_loss=True) -> np.ndarray:
        '''
        Compute the transfer matrix for the tunable MZI given the current values of theta, phi
        :param add_uncertainties: whether to include uncertainties in the transfer matrix computation
        :return: the transfer matrix
        '''
        if add_uncertainties:
            phi = self.phi + self.phi_err
            theta = self.theta + self.theta_err + (self.omega * self.delta_t)
        else:
            phi, theta = self.phi, (self.theta + (self.omega * self.delta_t))
    ...
    def get_partial_transfer_matrices(self, backward=False, cumulative=True, add_uncertainties=True, add_loss=True) -> np.ndarray:
        '''
        Compute the partial transfer matrices of each "column" of the MZI -- after first beamsplitter, after first
        phase shifter, after second beamsplitter, and after second phase shifter
        :param backward: if true, compute the reverse transfer matrices in backward order
    ...
    def randomize_errors(self):
        '''Randomize the loss and phase errors of the MZI'''
        
        self.loss_dB_cur = get_loss(self.loss_dB, loss_diff=self.loss_diff)
        self.phi_err = np.random.normal(0, self.phase_uncert_phi)
        self.theta_err = np.random.normal(0, self.phase_uncert_theta)
