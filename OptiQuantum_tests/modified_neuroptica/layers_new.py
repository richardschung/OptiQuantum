...
class NetworkLayer:
    ...
    def set_phases_uncert_loss(self, phases, phase_uncert_theta, phase_uncert_phi, loss_dB, loss_diff):
        ...
        if omegas is None:
            omegas = [None for _ in range(sum(mzi_nums))]
        if delta_ts is None:
            delta_ts = [None for _ in range(sum(mzi_nums))]
        ...
        for ii in mzi_nums:
            ...
            for jj in range(ii):
                ...
                omegas_layer.append(omegas[idx])
                delta_ts_layer.append(delta_ts[idx])
                idx += 1
            phases_mzi_layer.append(phases_layer)
            omegas_mzi_layer.append(omegas_layer)
            delta_ts_mzi_layer.append(delta_ts_layer)

        # create every layer of MZIs with new loss/phase uncerts
        layerCount = 0 # keep track of which layer we are creating
        for start, end, phases, omegas, delta_ts in zip(self.mzi_limits_lower, self.mzi_limits_upper, phases_mzi_layer, omegas_mzi_layer, delta_ts_mzi_layer):
            thetas = [phase[0] for phase in phases]
            phis = [phase[1] for phase in phases]
            layers.append(MZIDelayLayer.from_waveguide_indices(MZIDelayLayer, layerCount, self.N, list(range(start, end + 1)), thetas=thetas, phis=phis, phase_uncert_theta=phase_uncert_theta, phase_uncert_phi=phase_uncert_phi, loss_dB=loss_dB, loss_diff=loss_diff, omegas=omegas, delta_ts=delta_ts))
            layerCount += 1
        self.mesh = OpticalMeshNew(self.N, layers)
...
class ReckLayer_H(OpticalMeshNetworkLayer): # Hermitian Transpose of a Reck Layer
    ...
    def __init__(self, N: int, include_phase_shifter_layer=False, initializer=None, loss_dB=0, thetas=[None], phis=[None]):
        ...
        # Finally, create each MZI Layer (the hermitian transpose of the Reck layer's MZIDelayLayer)
        for start, end, phases, omegas, delta_ts in zip(mzi_limits_lower, mzi_limits_upper, phases_mzi_layer, omegas_mzi_layer, delta_ts_mzi_layer):
            thetas = [phase[0] for phase in phases]
            phis = [phase[1] for phase in phases]
            layers.append(MZIDelayLayer_H.from_waveguide_indices(None, N, list(range(start, end + 1)), loss_dB=loss_dB, thetas=thetas, phis=phis, omegas = omegas, delta_ts = delta_ts))

        self.mesh = OpticalMeshNew(N, layers)
...
class DiamondLayer(OpticalMeshNetworkLayer): 
    ...
    def __init__(self, N: int, include_phase_shifter_layer=False, initializer=None, phases=[(None, None)], loss_dB=0, loss_diff=0, phase_uncert=0.0, omegas=None, delta_ts=None):
        ...
        for start, end, phases, omegas, delta_ts in zip(self.mzi_limits_lower, self.mzi_limits_upper, phases_mzi_layer, omegas_mzi_layer, delta_ts_mzi_layer):
            thetas = [phase[0] for phase in phases]
            phis = [phase[1] for phase in phases]
            layers.append(MZIDelayLayer.from_waveguide_indices(MZIDelayLayer, layerCount, self.S, list(range(start, end + 1)), thetas=thetas, phis=phis, phase_uncert_theta=phase_uncert, phase_uncert_phi=phase_uncert, loss_dB=loss_dB, loss_diff=loss_diff, omegas=omegas, delta_ts=delta_ts))
            layerCount += 1

        self.mesh = OpticalMeshNew(S, layers)
    ...
    def set_phases_uncert_loss(self, phases, phase_uncert_theta, phase_uncert_phi, loss_dB, loss_diff):
        ...
        for start, end, phases, omegas, delta_ts in zip(self.mzi_limits_lower, self.mzi_limits_upper, phases_mzi_layer, omegas_mzi_layer, delta_ts_mzi_layer):
            thetas = [phase[0] for phase in phases]
            phis = [phase[1] for phase in phases]
            layers.append(MZIDelayLayer.from_waveguide_indices(MZIDelayLayer, layerCount, self.S, list(range(start, end + 1)), thetas=thetas, phis=phis, phase_uncert_theta=phase_uncert_theta, phase_uncert_phi=phase_uncert_phi, loss_dB=loss_dB, loss_diff=loss_diff, omegas=omegas, delta_ts=delta_ts))
            layerCount += 1
        self.mesh = OpticalMeshNew(self.S, layers)

class DMM_layer(OpticalMeshNetworkLayer):
    """ Diagonal Matrix Multiplication Layer """    

    def __init__(self, N: int, initializer=None, phases=[(None, None)], loss_dB=0, loss_diff=0, phase_uncert=0.0, omegas=None, delta_ts=None):
        '''
        ...
        if omegas is None:
            omegas = [None for _ in range(int(N*(N-1)/2))]
        if delta_ts is None:
            delta_ts = [None for _ in range(int(N*(N-1)/2))]
        mzi_nums = [int(len(range(start, end+1))/2) for start, end in zip(self.mzi_limits_lower, self.mzi_limits_upper)] # get the number of MZIs in this component layer
        ...
        for start, end, phases, omegas, delta_ts in zip(self.mzi_limits_lower, self.mzi_limits_upper, phases_mzi_layer, omegas_mzi_layer, delta_ts_mzi_layer):
            thetas = [phase[0] for phase in phases]
            phis = [phase[1] for phase in phases]
            layers.append(MZIDelayLayer.from_waveguide_indices(MZIDelayLayer, None, N, list(range(start, end + 1)), thetas=thetas, phis=phis, phase_uncert_phi=self.phase_uncert, phase_uncert_theta=self.phase_uncert, loss_dB=loss_dB, loss_diff=loss_diff, omegas=omegas, delta_ts=delta_ts))
...
class flipped_ReckLayer(OpticalMeshNetworkLayer):
    ...
    up rather than down, like in the original Reck mesh'''

    def __init__(self, N: int, include_phase_shifter_layer=False, initializer=None, phases=[(None, None)], shifter_phases=None, loss_dB=0, loss_diff=0, phase_uncert=0.0, omegas=None, delta_ts=None):
        '''
        Initialize the ReckLayer
        :param N: number of input and output waveguides
        :param include_phase_shifter_layer: if true, include a layer of single-mode phase shifters at the beginning of
        ...
        if include_phase_shifter_layer:
            shifter_layer = PhaseShifterLayer(N)
            if shifter_phases is not None:
                for i in range(len(shifter_layer.phase_shifters)):
                    shifter_layer.phase_shifters[i].phi = shifter_phases[i]
            layers.append(shifter_layer)
...
class ReckLayer(OpticalMeshNetworkLayer):
    ...
    def __init__(self, N: int, include_phase_shifter_layer=False, initializer=None, phases=[(None, None)], shifter_phases=None, loss_dB=0, loss_diff=0, phase_uncert=0.0, omegas=None, delta_ts=None):
        ...
        for start, end, phases, omegas, delta_ts in zip(self.mzi_limits_lower, self.mzi_limits_upper, phases_mzi_layer, omegas_mzi_layer, delta_ts_mzi_layer):
            thetas = [phase[0] for phase in phases]
            phis = [phase[1] for phase in phases]
            layers.append(MZIDelayLayer.from_waveguide_indices(MZIDelayLayer, layerCount, N, list(range(start, end + 1)), thetas=thetas, phis=phis, phase_uncert_phi=self.phase_uncert, phase_uncert_theta=self.phase_uncert, loss_dB=loss_dB, loss_diff=loss_diff, omegas=omegas, delta_ts=delta_ts))
            layerCount += 1
...
class ClementsLayer(OpticalMeshNetworkLayer):
    ...
    def __init__(self, N: int, M=None, include_phase_shifter_layer=False, initializer=None, phases=[(None, None)], shifter_phases=None, loss_dB=0, loss_diff=0, phase_uncert=0.0, omegas=None, delta_ts=None):
        '''
        ...
        if N % 2 == 0:
            self.mzi_limits_lower = [(i) % 2 for i in range(self.M)]
            self.mzi_limits_upper = [N - 1 - (i) % 2 for i in range(self.M)]
        else:
            self.mzi_limits_lower = [(i) % 2 for i in range(self.M)]
            self.mzi_limits_upper = [N - 1 - (i + 1) % 2 for i in range(self.M)]
        ...
        for start, end, phases, omegas, delta_ts in zip(self.mzi_limits_lower, self.mzi_limits_upper, phases_mzi_layer, omegas_mzi_layer, delta_ts_mzi_layer):
            thetas = [phase[0] for phase in phases]
            phis = [phase[1] for phase in phases]
            # layers.append(MZIDelayLayer.from_waveguide_indices(MZIDelayLayer, N, list(range(start, end + 1)), thetas=thetas, phis=phis, phase_uncert=self.phase_uncert, loss_dB=loss_dB, loss_diff=loss_diff, omegas=omegas, delta_ts=delta_ts))
            layers.append(MZIDelayLayer.from_waveguide_indices(MZIDelayLayer, layerCount, self.N, list(range(start, end + 1)), thetas=thetas, phis=phis, phase_uncert_theta=phase_uncert, phase_uncert_phi=phase_uncert, loss_dB=loss_dB, loss_diff=loss_diff, omegas=omegas, delta_ts=delta_ts))
            layerCount += 1
