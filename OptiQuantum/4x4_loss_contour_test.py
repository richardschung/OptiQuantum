...
def main():
    ...
    max_diff = 0.8
    step_size_diff = 0.1
    n_diff = int(max_diff/step_size_diff) + 1
    losses_diff = np.linspace(0,0.8,n_diff)

    visibs = np.zeros([n_diff,n_dB])
    temp_visibs = np.zeros(5)
    
    for i in range(n_dB):
        for j in range(n_diff):
            for layer in mesh.layers:
                for mzi in layer:
                    mzi.loss_diff = losses_diff[j]
                    mzi.loss_dB = losses_dB[i]
            ...
            for k in range(5):
                ...
                temp_visibs[k] = (coinc_max[0] - coinc_min[0])*2

            visibs[i,j] = np.mean(temp_visibs)
    ...
    plt.pcolor(losses_dB, losses_diff, visibs.T, cmap=cmap, rasterized=True)
    ...
    plt.ylabel('Loss/MZI (diff)', fontsize=labels_size)
    cbar = plt.colorbar()
