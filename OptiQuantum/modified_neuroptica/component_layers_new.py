...
class PhaseShifterLayer(ComponentLayer):
    ...
    def get_transfer_matrix(self, add_uncertainties=False, add_loss=False) -> np.ndarray:
...
class OpticalMesh:
    ...
    def compute_phase_shifter_fields(self, X: np.ndarray, align="right", use_partial_vectors=False, include_bs=False) -> \
        ...
        for layer in self.layers:
            #Add compatibility with new MZI layer
            if isinstance(layer, MZILayer) | isinstance(layer, MZIDelayLayer):
    ...
    def adjoint_optimize(self, forward_field: np.ndarray, adjoint_field: np.ndarray,
        ...
        for layer, layer_fields, layer_fields_adj in zip(self.layers, forward_fields, reversed(adjoint_fields)):
            ...
            #Add compatibility with new MZI layer
            elif isinstance(layer, MZILayer) | isinstance(layer, MZIDelayLayer):
...
class MZIDelayLayer(MZILayer):
    def from_waveguide_indices(cls, p_layerCount: int, N: int, waveguide_indices: List[int], thetas=None, phis=None, phase_uncert_theta=0.0, phase_uncert_phi=0.0, loss_dB=0, loss_diff=0, omegas=None, delta_ts=None):
        '''
        Create an MZI layer from a list of an even number of input/output indices. Each pair of waveguides in the
        iteration order will be assigned to an MZI
        :param N: size of MZILayer
        :param waveguide_indices: list of waveguides the layer attaches to
        :return: MZILayer class instance with size N and delaying MZIs attached to waveguide_indices
        '''
        if thetas is None:
            thetas = [thetas]*int(len(waveguide_indices)/2)
        if phis is None:
            phis = [phis]*int(len(waveguide_indices)/2)
        if omegas is None:
            omegas = np.zeros(int(len(waveguide_indices)/2))
        if delta_ts is None:
            delta_ts = np.zeros(int(len(waveguide_indices)/2))
        ...
        for i in range(0, len(waveguide_indices), 2):
            ...
            '''1. All MZIs have the same uncertainties'''
            mzis.append(MZI_delay(waveguide_indices[i], waveguide_indices[i + 1], theta=thetas[len(mzis)], phi=phis[len(mzis)], phase_uncert_theta=phase_uncert_theta, phase_uncert_phi=phase_uncert_phi, loss_dB=loss_dB, loss_diff=loss_diff, omega=omegas[len(mzis)], delta_t=delta_ts[len(mzis)]))
    ...
    def get_transfer_matrix(self, add_uncertainties=True, add_loss=True) -> np.ndarray:
        T = np.eye(self.N, dtype=NP_COMPLEX)
        for mzi in self.mzis:
            U = mzi.get_transfer_matrix(add_uncertainties, add_loss)
            m, n = mzi.m, mzi.n
            T[m][m] = U[0, 0]
            T[m][n] = U[0, 1]
            T[n][m] = U[1, 0]
    ...
    def get_partial_transfer_matrices(self, backward=False, cumulative=True, add_uncertainties=False, add_loss=True) -> np.ndarray:
        '''
        Return a list of 4 partial transfer matrices for the entire MZI layer corresponding to (1) after first BS in
        each MZI, (2) after theta shifter, (3) after second BS, and (4) after phi shifter. Order is reversed in the
        backwards case
        ...
        # Compute the (non-cumulative) partial transfer matrices for each MZI
        all_mzi_partials = [mzi.get_partial_transfer_matrices
                            (backward=backward, cumulative=False, add_uncertainties=add_uncertainties, add_loss=add_loss)
                            for mzi in self.mzis]
        # print(all_mzi_partials)
    ...
    def get_partial_transfer_vectors(self, backward=False, cumulative=True, add_uncertainties=False, add_loss=True) -> np.ndarray:
        '''
        :param backward:
        :param cumulative:
        :param add_uncertainties:
    ...
    def get_indiv_transfer_matrices(self, add_uncertainties=True, add_loss=True) -> np.ndarray:
        return [mzi.get_transfer_matrix() for mzi in self.mzis]
    
    def adjust_omegas(self,omega_new):
        for mzi in self.mzis:
            mzi.omega = omega_new
        return

class OpticalMeshNew(OpticalMesh):
    def get_transfer_matrix(self, add_uncertainties=True, add_loss=True) -> np.ndarray:
        return reduce(np.dot, [layer.get_transfer_matrix(add_uncertainties, add_loss) for layer in reversed(self.layers)])
        #return reduce(np.dot, [layer.get_transfer_matrix(add_uncertainties, add_loss) for layer in self.layers])
