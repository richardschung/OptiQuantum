...
def main():
    ...
    N = 4
    #Create Mesh
    network = ReckLayer(N, phases=[(np.pi, np.pi) for _ in range(int(N*(N-1)/2))])
    mesh = network.mesh
    ...
    for layer in mesh.layers:
        for mzi in layer:
            ...
            plt.savefig(f'figures_set1/fidelity_vs_random_phases_Reck_1000samples_MZI_{n}.png')

            #Save Data
            np.savetxt(f'figures_set1/fidelity_vs_random_phases_Reck_1000samples_MZI_{n}.txt',fidels)
