...
def main():
    ...
    transmittances = np.zeros([n_dB,n_Haars])
    #out1 = 0
    #out2 = 1
    out1 = N-2
    out2 = N-1
    ...
    for haar_num in range(n_Haars):
        ...
        network = ClementsLayer(N, phases=phase_data, include_phase_shifter_layer=True,shifter_phases=localv,loss_diff=0.00)
        mesh = network.mesh
        print(np.round(matrix,2))
        print(np.round(mesh.get_transfer_matrix(),2))

        print(np.round(abs(mesh.get_transfer_matrix()-matrix),2))
        return
                    ...
                    for mzi_cur in layer_cur:
                        if isinstance(mzi_cur,MZI_delay):
                            ...
                            if mzi_cur.loss_dB_cur < 0:
                                ...
                                print("gain detected")
                transmittance_samples[sample] = fidelity(mesh,matrix)
            transmittances[i,haar_num] = np.mean(transmittance_samples)
            #print(fidels[i,j])
    ...
    #plt.savefig(f'figures_set1/transmittance_7_8_vs_random_loss_Clements_1000_samples_1Haar_loss_diff_002.png')

    #Save Data
    #np.savetxt(f'figures_set1/transmittance__7_8_vs_random_loss_Clements_1000samples_1Haar_loss_diff_002.txt',transmittances)
    timer.toc()
