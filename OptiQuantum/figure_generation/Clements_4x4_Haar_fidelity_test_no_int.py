...
def main():
    ...
    fidels = np.zeros([n_phase,n_phase])
    ...
    for i in range(n_phase):
        for j in range(n_phase):
            ...
            fidels[i,j] = np.mean(fidel_samples)
            #print(fidels[i,j])
    ...
    plt.savefig(f'figures_set1/fidelity_vs_random_phases_Clements_1000samples_Haar.png')

    #Save Data
    np.savetxt(f'figures_set1/fidelity_vs_random_phases_Clements_1000samples_Haar.txt',fidels)
    np.savetxt(f'figures_set1/fidelity_vs_random_phases_Clements_1000samples_Haar_ideal.txt',matrix)
