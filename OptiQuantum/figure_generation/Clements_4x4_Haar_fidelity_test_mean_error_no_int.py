...
from strawberryfields import rectangular_symmetric
...
def main():
    ...
    for haar_num in range(n_Haars):
        ...
        network = ClementsLayer(N, phases=phase_data, include_phase_shifter_layer=True,shifter_phases=localv,phase_uncert=0)
        mesh = network.mesh

        print(np.round(matrix-mesh.get_transfer_matrix(),2))
        return
    ...
    plt.savefig(f'figures_set1/fidelity_vs_mean_phases_Clements_1000samples_5Haars_unc_pi_32.png')

    #Save Data
    np.savetxt(f'figures_set1/fidelity_vs_mean_phases_Clements_1000samples_5Haars_unc_pi_32.txt',fidels)
