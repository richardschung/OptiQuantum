...
def main():
    ...
    N = 4
    #Create Mesh
    network = DiamondLayer(N, phases=[(np.pi, np.pi) for _ in range(int((N-1)**2))])
    mesh = network.mesh
    print(np.round(mesh.get_transfer_matrix(),2))
    print(np.shape(mesh.layers))
    ...
    for layer in mesh.layers:
        for mzi in layer:
            ...
            plt.savefig(f'figures_set1/fidelity_vs_random_phases_Bokun_1000samples_MZI_{n}.png')

            #Save Data
            np.savetxt(f'figures_set1/fidelity_vs_random_phases_Bokun_1000samples_MZI_{n}.txt',fidels)
