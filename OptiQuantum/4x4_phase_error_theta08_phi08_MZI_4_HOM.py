...
def main():
    ...
    layers[2].mzis[0].theta = np.pi/2
    layers[2].mzis[0].phi = 0
    ...
    for i in range(res):
        ...
        intfunc = lambda omega1, omega2:HOM_integrand_mesh(mesh,omega1,omega2,taus[i], 1, 0, 1, 0, 1)
    ...
    plt.xlabel(r'$\tau$ (1/$\sigma$)', fontsize=labels_size)
    plt.ylabel(r'Coincidence Rate', fontsize=labels_size)
    #cbar = plt.colorbar()
    ...
    plt.savefig(f'4x4_Clements/4x4_phase_error_theta08_phi08_Clements_MZI_4.png')
