...
def main():
    ...
    layers[0].mzis[1].theta = np.pi/2
    layers[0].mzis[1].phi = 0
    ...
    for i in range(n_dB):
        ...
        intfunc_min = lambda omega1, omega2:HOM_integrand_mesh(mesh,omega1,omega2,0, 1, 2, 3, 2, 3)
        coinc_min = scipy.integrate.dblquad(intfunc_min,-np.inf,np.inf,-np.inf,np.inf)

        intfunc_max = lambda omega1, omega2:HOM_integrand_mesh(mesh,omega1,omega2,5, 1, 2, 3, 2, 3)
        coinc_max = scipy.integrate.dblquad(intfunc_max,-np.inf,np.inf,-np.inf,np.inf)

        visibs[i] = (coinc_max[0] - coinc_min[0])*2

    fig, ax = plt.subplots()
    ax.plot(losses_dB, visibs)
