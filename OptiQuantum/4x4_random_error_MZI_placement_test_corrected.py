...
def main():
    ...
    losses_diff = np.linspace(0,max_dB,n_dB)
    max_phase = 0.8
    step_size_phase = 0.1
    ...
    for layer in mesh.layers:
        for mzi in layer:
                    ...
                    for sample in range(n_samples):
                        for layer_cur in mesh.layers:
                            for mzi_cur in layer_cur:
                                mzi_cur.loss_diff = losses_diff[i]
                                mzi_cur.phase_uncert_theta = phases[j]
            ...
            plt.savefig(f'4x4_Clements/4x4_Clements_MZI_{n}.png')
