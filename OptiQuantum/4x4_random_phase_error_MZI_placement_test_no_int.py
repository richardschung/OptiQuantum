'''Program to test HOM visibility simulation using Neuroptica layers.
Current layer to test is an MZI mesh with one component as a beam splitter and the rest of the components in bar state.
'''
import numpy as np
import scipy
...
def main():
    ...
    for layer in mesh.layers:
        for mzi in layer:
            ...
            for i in range(n_theta):
                for j in range(n_phi):
                    for sample in range(n_samples):
                        ...
                        #print("Visibility = " + str(visib_samples[sample]))
            ...
            plt.savefig(f'4x4_Clements/4x4_random_phase_Clements_no_int_MZI_{n}.png')
