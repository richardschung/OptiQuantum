...
def main():
    ...
    max_theta = 0.8
    step_size_theta = 0.1
    n_theta = int(max_theta/step_size_theta) + 1
    thetas = np.linspace(0,max_theta,n_theta)
    max_phi = 0.8
    step_size_phi = 0.1
    n_phi = int(max_phi/step_size_phi) + 1
    phis = np.linspace(0,max_phi,n_phi)

    visibs = np.zeros([n_theta,n_phi])
    ...
    for layer in mesh.layers:
        for mzi in layer:
                    ...
                    for sample in range(n_samples):
                        for layer_cur in mesh.layers:
                            for mzi_cur in layer_cur:
                                ...
                                mzi_cur.phase_uncert_theta = thetas[i]
                                mzi_cur.phase_uncert_phi = phis[j]
                                mzi_cur.randomize_errors()
                                
                        intfunc_min = lambda omega1, omega2:HOM_integrand_mesh(mesh,omega1,omega2,0, 1, mzi.m, mzi.n, mzi.m, mzi.n)
                        coinc_min = scipy.integrate.dblquad(intfunc_min,-np.inf,np.inf,-np.inf,np.inf)

                        intfunc_max = lambda omega1, omega2:HOM_integrand_mesh(mesh,omega1,omega2,5, 1, mzi.m, mzi.n, mzi.m, mzi.n)
                        coinc_max = scipy.integrate.dblquad(intfunc_max,-np.inf,np.inf,-np.inf,np.inf)

                        visib_samples[sample] = (coinc_max[0]-coinc_min[0])/coinc_max[0]
            ...
            plt.pcolor(thetas, phis, visibs.T, cmap=cmap, rasterized=True, vmin=0, vmax = 1)
            ...
            plt.xlabel(r'$\theta$ uncertainty (radians)', fontsize=labels_size)
            plt.ylabel(r'$\phi$ uncertainty (radians)', fontsize=labels_size)
            cbar = plt.colorbar()
            ...
            plt.savefig(f'4x4_Clements/4x4_random_phase_Clements_MZI_{n}.png')
